...
use crate::widgets::{
    self, add_line, label, label_right, CONTENT_WIDTH, PAD_X, POPOVER_WIDTH,
};
...
pub fn build(state: &AppState, theme: &Theme, target: &AnyObject, mtm: MainThreadMarker) -> (Retained<NSView>, f64) {
    ...
    draw_clean_header(&root, height, state, freed, done, total, theme, mtm);
    let y = draw_removing_path(&root, height - header::HEADER_HEIGHT, &current, theme, mtm);
    if let Some(p) = progress {
        let _ = draw_items(&root, y, p, theme, mtm);
    }
    draw_clean_footer(&root, theme, target, mtm);
    (root, height)
}

#[allow(clippy::too_many_arguments)]
fn draw_clean_header(
    root: &NSView,
    height: f64,
    state: &AppState,
    freed: u64,
...
fn draw_clean_footer(root: &NSView, theme: &Theme, target: &AnyObject, mtm: MainThreadMarker) {
...
fn draw_items(
    ...
    for item in &progress.items {
        ...
        let (mark, color, _strike) = match item.status {
            CleanItemStatus::Done => ("\u{2713}", theme.pos, true),
