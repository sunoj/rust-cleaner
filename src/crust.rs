...
// one patch of it is painted. Kept apart from the plate itself because it is
// the only part that has to be true to scale — the legend quotes the same
// figure this sizes the region from.
// Exports: `crust_region`, `draw_tile`, `phase`.
// Deps: crate::{metal, treemap, widgets}, objc2 AppKit, wd40::disk.

use crate::metal::{inset, outline, rnd};
